...
//2.-FUNCIÓN PARA ADMINISTRAR LA CALIFICACIÓN
  handleRatings: async function (dish) {
    ...
    var orderSummary = await this.getOrderSummary(tNumber);
    //Guardar solo el campo current order
    var sgco=Object.keys(orderSummary.current_orders)
    //Checar si hay algo guardado en el campo y si esta el id del platillo que se desea calificar
    if(sgco.length>0 && sgco==dish.id){
      // Cerrar el modal
    document.getElementById("rating-modal-div").style.display="flex"
    document.getElementById("rating-input").value="0"
    ocument.getElementById("feedback-input").value=""
      // Evento de clic del botón de envio
      var be=document.getElementById("save-rating-button")
      be.addEventListener("click",()=>{
      document.getElementById("rating-modal-div").style.display="none"
      var rating=document.getElementById("rating-input").value
      var cs=document.getElementById("feedback-input").value
      ...
      .doc(dish.id)
      .update({
        last_review:cs,last_rating:rating
      })
      .then(()=>{
        swal({
          icon: "success",
          title: "¡Gracias por su comment!",
          text: "vuelva pronto",
          timer: 2500,
          buttons: false
        });
      ...
      swal({
        icon: "warning",
        title: "oops",
        text: "noo has comprado esta comida",
        timer: 2500,
        buttons: false
      });
    }
